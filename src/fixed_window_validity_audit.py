...
def native_numeric_cadence(track) -> float:
    """Estimate cadence from native record timestamps; return NaN if not estimable."""
    ...
    return float(np.median(gaps)) if len(gaps) else np.nan
...
def numeric_metrics(
    ...
    cadence_estimable = bool(np.isfinite(cadence) and cadence > 0)
    if cadence_estimable:
        ...
        base_observed_coverage = 0.0
        out_observed_coverage = 0.0
    ...
    longest = longest_run_seconds(valid_base, cadence) if cadence_estimable else 0.0
...
def build_summary(data: pd.DataFrame, grid: pd.DataFrame) -> dict:
    ...
    model_viable = (grid.events >= 10) & (grid.non_events >= 10)
    overlap = data[data.numeric_raw_overlap_seconds >= 30]
    cadence = data.numeric_native_cadence_s.dropna()
    summary = {
        ...
        "all_192_definition_grid": {
            "definitions_n": int(len(grid)),
            "retained_min_n": int(grid.retained_n.min()),
            "retained_max_n": int(grid.retained_n.max()),
            "events_min_n": int(grid.events.min()),
            "events_max_n": int(grid.events.max()),
            "non_events_min_n": int(grid.non_events.min()),
            "non_events_max_n": int(grid.non_events.max()),
            "definitions_reaching_10_events_and_10_non_events_n": int(model_viable.sum()),
        },
        ...
        "numeric_native_cadence_s": {
            "numeric_track_present_n": int(data.numeric_track_present.fillna(False).sum()),
            "estimable_n": int(len(cadence)),
            "not_estimable_n": int(len(data) - len(cadence)),
            "nominal_fallback_n": 0,
            "median": float(cadence.median()) if len(cadence) else None,
            "minimum": float(cadence.min()) if len(cadence) else None,
            "maximum": float(cadence.max()) if len(cadence) else None,
            "within_0_05_s_of_2_s_n": int(
                np.isclose(cadence, 2.0, atol=0.05).sum()
            ),
        },
    }
    for amp_threshold in RAW_PULSE_AMPLITUDES:
