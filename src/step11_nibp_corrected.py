...
from project_paths import METRICS_DIR, PHYSIONET_ROOT, PROCESSED_DIR, PROJECT_ROOT, VITAL_DIR
...
def load_nibp_1hz(caseid, t0):
    try:
        path = VITAL_DIR / f"{int(caseid):04d}.vital"
        vf = vitaldb.VitalFile(str(path), track_names=['Solar8000/NIBP_MBP'])
        d = vf.to_numpy(['Solar8000/NIBP_MBP'], interval=1)
        if d is None or len(d) == 0:
            return None, 0
        arr = np.array(d).flatten().astype(float)
        arr[(arr < MAP_MIN) | (arr > MAP_MAX)] = np.nan
        track = next(
            (track for track in vf.trks.values() if track.dtname == 'Solar8000/NIBP_MBP'),
            None,
        )
        native_baseline_n = 0
        if track is not None:
            for rec in track.recs:
                relative_time = float(rec['dt']) - float(vf.dtstart)
                value = float(rec['val'])
                if t0 - BASELINE_SECS <= relative_time < t0 and MAP_MIN <= value <= MAP_MAX:
                    native_baseline_n += 1
        return arr, native_baseline_n
...
native_counts = nibp.loc[nibp.nibp_baseline.notna(), 'native_baseline_readings']
...
def paired_boot_delta(y, p_new, p_ref, n=2000, seed=42):
    """Paired patient-level bootstrap CI for an AUROC difference."""
    rng = np.random.default_rng(seed); deltas = []
    for _ in range(n):
        i = rng.choice(len(y), len(y), replace=True)
        if np.unique(y[i]).size < 2:
            continue
        deltas.append(roc_auc_score(y[i], p_new[i]) - roc_auc_score(y[i], p_ref[i]))
...
for name, fs in specs:
    ...
    _, lo, hi = boot_auroc(y[ite], p)
    au = roc_auc_score(y[ite], p)
...
delta_m1_m0 = float(
    roc_auc_score(y[ite], predictions["M1: Clinical + MAP"])
    - roc_auc_score(y[ite], predictions["M0: Clinical only"])
)
delta_m3_m1 = float(
    roc_auc_score(y[ite], predictions["M3: Clinical + MAP + RI"])
    - roc_auc_score(y[ite], predictions["M1: Clinical + MAP"])
)
delta_m1_m0_ci = paired_boot_delta(
    y[ite], predictions["M1: Clinical + MAP"], predictions["M0: Clinical only"], seed=421
)
delta_m3_m1_ci = paired_boot_delta(
    y[ite], predictions["M3: Clinical + MAP + RI"], predictions["M1: Clinical + MAP"], seed=422
)
...
summary = dict(
    ...
    native_baseline_readings={
        "median": round(float(native_counts.median()), 1),
        "iqr": [round(float(native_counts.quantile(.25)), 1), round(float(native_counts.quantile(.75)), 1)],
        "one_reading_n": int((native_counts == 1).sum()),
        "two_readings_n": int((native_counts == 2).sum()),
        "three_or_more_readings_n": int((native_counts >= 3).sum()),
    },
    ...
    paired_delta_auroc={
        "M1_minus_M0": {
            "estimate": round(delta_m1_m0, 3),
            "ci95": [round(float(x), 3) for x in delta_m1_m0_ci],
        },
        "M3_minus_M1": {
            "estimate": round(delta_m3_m1, 3),
            "ci95": [round(float(x), 3) for x in delta_m3_m1_ci],
        },
    },
)
